...
function oddsAndEvens(array) {
  const evenArray = array.map (function (number) {
    if (number % 2 === 0) {
      return "even" 
    }
    return "odd" 
...
function beginsWithB(array) {
  const makeLarge = array.filter (function (startsWithB) {
    return startsWithB.startsWith("B")
  })
  return makeLarge
}
...
const timesTen = array.map (function (multi10) {
  return Math.abs(multi10) * 10
  })
  return timesTen
}
