...
function stringsOnly(array) {
  ...
  const stringToArray = array.filter(function (arrayItem) {
...
function convertTemps(array) {
  const temp = array.map(function (changeTemp) {
    return (changeTemp - 32) * 5 / 9
  })
return temp
}
...
function characterCount(array) {
  ...
  const wordLengths =  array.map(function (word) {
    return word.length
  })
  return wordLengths
...
return array.filter(wLength => wLength.length >= minLength)
}
